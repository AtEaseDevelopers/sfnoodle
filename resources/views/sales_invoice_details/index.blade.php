...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">Sales Invoice Details</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             <!-- <a class="pull-right" href="{{ route('salesInvoiceDetails.create') }}"><i class="fa fa-plus-square fa-lg"></i></a> -->
                             <a class="pull-right text-danger pr-2" id="massdelete" href="#" alt="Mass delete"><i class="fa fa-trash fa-lg"></i></a>
                         </div>
                         <div class="card-body">
                             @include('sales_invoice_details.table')
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/salesInvoiceDetails/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/salesInvoiceDetails/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/salesInvoiceDetails/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status
